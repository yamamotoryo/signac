...
my_CallPeaks.Seurat <- function(
  ...
  if (!is.null(x = group.by)) {
    # first check macs3 path before we spend time splitting the files
    macs3.path <- SetIfNull(
      x = macs3.path,
      y = unname(obj = Sys.which(names = "macs3"))
    )
    if (nchar(x = macs3.path) == 0) {
      stop("MACS3 not found. Please install MACS:",
...
#' @method my_CallPeaks ChromatinAssay
#' @rdname my_CallPeaks
#' @concept quantification
#' @export
my_CallPeaks.ChromatinAssay <- function(
    object,
    macs3.path = NULL,
    outdir = tempdir(),
    broad = FALSE,
...
#' @method my_CallPeaks Fragment
#' @rdname my_CallPeaks
#' @concept quantification
#' @export
my_CallPeaks.Fragment <- function(
    object,
    macs3.path = NULL,
    outdir = tempdir(),
    broad = FALSE,
...
#' @method my_CallPeaks default
#' @rdname my_CallPeaks
#' @concept quantification
#' @export
my_CallPeaks.default <- function(
    object,
    macs3.path = NULL,
    outdir = tempdir(),
    broad = FALSE,
